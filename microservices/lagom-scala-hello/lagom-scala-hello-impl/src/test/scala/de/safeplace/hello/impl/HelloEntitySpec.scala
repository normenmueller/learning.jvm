...
class HelloEntitySpec extends WordSpec with Matchers with BeforeAndAfterAll {
  ...
  private val system = ActorSystem("LagomscalahelloEntitySpec",
  ...
  private def withTestDriver(block: PersistentEntityTestDriver[HelloCommand[_], HelloEvent, HelloState] => Unit): Unit = {
    val driver = new PersistentEntityTestDriver(system, new HelloEntity, "lagom-scala-hello-1")
    block(driver)
    driver.getAllIssues should have size 0
  }

  "lagom-scala-hello entity" should {
